		...
		egui::Window::new("Emoji picker")
			...
			.show(ui.ctx(), |ui| {
				...
				ui.horizontal(|ui| {
					...
					.response
					.on_hover_text(
						"Custom emoji from the current server; cross-server use is not verified.",
					);
				});
				ui.separator();
				...
				if self.server {
					...
					let matching: Vec<_> = emojis
						...
						.collect();
					ui.weak(
						"Restricted or unavailable emoji are disabled. Animated emoji use a still preview.",
					);
					if matching.is_empty() {
						ui.weak(if emojis.is_empty() {
							...
							for row in rows {
								ui.horizontal(|ui| {
									for emoji in matching.iter().skip(row * columns).take(columns) {
										let button = if let Some(image) = avatars.custom_image(
											ui.ctx(),
											emoji.id,
											26.0,
											state.demo,
										...
										let response = ui
											.add_enabled_ui(emoji.usable(), |ui| {
												ui.add_sized([CELL, CELL], button)
											})
											.inner;
										...
										response.on_hover_text(&emoji.name).on_disabled_hover_text(
											format!(
												"{} — unavailable or permission not verified",
												emoji.name
											),
										);
						...
						.show_rows(
							...
							|ui, rows| {
								for row in rows {
									ui.horizontal(|ui| {
										for &index in
											self.matches.iter().skip(row * columns).take(columns)
										{
											let (text, name) = standard()[index];
											let button = if let Some(image) =
												crate::emoji::image(ui.ctx(), text, 26.0)
											{
