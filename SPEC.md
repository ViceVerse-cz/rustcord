...
Use egui/eframe for the desktop messaging UI. **Owner-authorized exception:** a temporary platform webview displays Discord’s real login page and its available email/password, QR, MFA, and other authentication options. Close and release the webview once authentication finishes or is canceled; it must not render the messaging application or remain a hidden runtime dependency. No Electron/Tauri wrapper, JavaScript messaging frontend, official Discord desktop process underneath, or local RPC dependency. System webview runtimes are permitted solely for this login flow; account for their platform dependencies and temporary resource usage.
...
Camera video and screen sharing are later capabilities. When added, use the appropriate OS capture mechanisms and the Discord-compatible transport/encryption path. Do not bundle large video dependencies into the text-only build preemptively. No recording or audio/video file cache by default.
