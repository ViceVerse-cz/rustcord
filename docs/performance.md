...
Isolated comparison: baseline `690ce91` to task commit `0477d06`, before integrating
`main` at `f01aeb6`. Clean task branch `t3code/fix-unread-message-banner`, Rust 1.98.1
...
the reported after samples use the isolated task implementation at `0477d06`. Foreground input and other
...
Integration with `main` at `f01aeb6` retained this fix and rebuilt both executables:
text 51,705,104 bytes; installed text package 58,956,832 bytes (686 files); sorted
DEFLATE-9 ZIP 36,670,157 bytes; voice executable 57,350,800 bytes. Voice packaging
still fails for the same four missing license texts. These sizes include upstream
presence/tray, mention and upload changes and are not attributed to the banner fix.
Integrated artifact/source hashes are in the JSON record. Native process/viewport
comparison was not repeated on the integrated tree; the table above describes only
the isolated task commit, not a performance guarantee for the merge.
