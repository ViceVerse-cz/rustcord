...
An optional People pane shows the active conversation’s members or DM participants. Narrow windows use a compact People window; profile clicks show only available account metadata. Member data is bounded and never represented as a complete guild directory when only a partial window is known.
