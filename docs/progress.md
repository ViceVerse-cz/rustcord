...
Verification: `cargo xtask check` passed (format, strict workspace/all-feature Clippy, workspace tests, text-only check, policy). After final status/UI refinements, focused strict Clippy and full tests for `discord-voice` and `ui` passed again. Coverage includes source/settings bounds, synthetic H264 encoding, actual MLS/DAVE encrypted H264 packetization/depacketization round trip, Gateway stream lifecycle, and coordinator startup-failure/permission-loss cleanup without devices or network. Two pre-existing echo-buffer Clippy findings were resolved with equivalent fixed-array chunk iteration. The first native test launch exposed a missing system Swift runtime rpath; build scripts now supply it to tests and the optional desktop binary. An initial release attempt reused stale model output after the baseline worktree build; scoped workspace release artifacts were cleared, then both final packages built successfully.

Independent reviews covered transport/Gateway/UI/desktop and native capture/encoder in disjoint reviewer scopes; participant bounds, stale keyframe handling, startup cleanup, permission changes and capture usage/init were corrected. `cargo xtask package-voice` and `cargo xtask package` produced locally ad-hoc signed macOS packages; neither is notarized. Synthetic before/after images are in `docs/pr-evidence/screen-sharing`. The dialog and high-quality selection were inspected natively. The preview did not consistently repaint after automation-driven resize/key events, so complete native narrow/keyboard verification remains unverified; no such pass is claimed. Windows API usage was reviewed against the pinned crate, but native Windows execution and owner-controlled Discord viewing remain open gates. No live account, screen recording or microphone was used for agent validation.

Integration: merged main `afac0ee`, preserving its independent UI/upload changes and resolving only the progress-log conflict by keeping both entries. Rechecking exposed a real microphone recovery edge: after a scheduling gap, the stall guard could discard the first frame queued by Ready. The capture clock now exempts the first enabled tick after resume/rekey/membership changes; the existing loopback fixture forces a 100 ms scheduling pause and passed six focused repetitions. The post-merge `cargo xtask check` then passed in full; both macOS package variants were rebuilt after integration. Performance/screenshots isolate feature commit `fb3736d`, before this integration and recovery fix.
...
Integration follow-up: merged `origin/main` at `f01aeb6`, preserving both sets of
appended docs and the upstream upload/mention/presence changes. Removed the duplicate
wheel phase field produced because both branches independently repaired that fixture.
The task diff against this main remains confined to timeline behavior/tests and evidence.

- On the integrated tree, the same Cargo focused commands pass: 24 timeline tests and
  one unread-navigation integration test. Formatting, policy and diff checks pass.
- Full `cargo xtask check` now advances past formatting but stops on existing voice
  camera Clippy lints: constant-size chunk iteration at lines 169/363 and collapsible
  conditions at lines 200/201. No camera code changed in this task.
- Full UI tests abort at the typing-label assertion in `crates/ui/src/typing.rs:285`.
  Reproduced the same failure with the unmodified `origin/main` timeline, restored
  the task file, and reran the focused tests successfully.
- Both integrated release executables build and the text package succeeds. Voice
  packaging remains blocked by the same four license-text omissions.
- Original CI run `34630749000` confirmed the old formatting failure and an unrelated
  stale `fuzz/Cargo.lock` (`--locked` cannot update it). The merged head gets fresh CI;
  pending checks are not reported as passed.

Performance comparisons remain pinned to isolated task commit `0477d06`; separate
integrated package sizes and the native evidence limitation are documented in
`docs/performance.md`. PR: https://github.com/ViceVerse-cz/Serein/pull/83 (draft).
